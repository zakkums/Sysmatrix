...
def test_json_redacts_sensitive_fields() -> None:
    snapshot = Snapshot(
        system=SystemData(
            user="alice",
            hostname="devbox",
        ...
        cpu=CpuData(model="cpu", cores=8, usage_percent=20.0),
        memory=MemoryData(
            total_gb=16.0,
            used_gb=4.0,
            usage_percent=25.0,
    ...
    rendered = render_json(snapshot, RuntimeConfig(opsec=True, plain=True, json=True))
    assert '"user": "[REDACTED]"' in rendered
    assert '"hostname": "[REDACTED]"' in rendered
    assert '"ip": "[REDACTED]"' in rendered
